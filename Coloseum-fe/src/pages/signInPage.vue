...
<script>
import imageGen from 'images-generator';
...
export default {
  ...
  data() {
    return {
      ...
      resignName: '',
      avatar: '',
      mascots: ['cat', 'dog', 'panda', 'bird', 'fox', 'shiba'],
    };
  },
  methods: {
    async onSubmit() {
      const randomScore = this.generateScore(110, 95);
      const animalFn = imageGen.animal[this.avatar];
      const imgUrl = await animalFn();
